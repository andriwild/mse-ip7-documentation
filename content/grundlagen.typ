...
- Nachteile: Die Flexibilität ist im Vergleich zu anderen Frameworks ist eingeschränkt. Zudem verwendet Ultralytics eine AGPL-3.0 Lizenz, was bei der Open Source Community nicht gut ankommt
...
Während auf eine GPU grundsätzlich keine Vorbedingungen an des Model gestellt werden, ist dies bei der TPU und NPU durchaus notwendig. Dabei spielen zwei Strategien eine wichtige Rolle: Quantisieren und Pruning @noauthor_sparsification_nodate. Beim Quantisieren werden die Fliesskommazahlen der Gewichte zu Ganzzahlen konvertiert, wodurch die Präzision abnimmt. Die reduziert den Speicherbedarf und ermöglicht eine schnellere Ausführung, weil weniger Rechenoperationen sind. Beim Pruning werden unwesentliche Verbindungen im Modell Netz entfernt, wodurch sich wiederum die Anzahl Rechenoperationen reduziert. Die Ansätze werden auch kombiniert angewendet.
...
Die NPU @noauthor_what_2024 kann ML-Tasks hochgradig parallelisieren und effizient auszuführen. In bestimmten Szenarien kann eine NPU eine GPU in Sachen Geschwindigkeit deutlich übertreffen @noauthor_npu_2024.
Die NPU basiert auf der Funktionsweise neuronaler Netzwerke ähnlich wie im menschlichen Gehirn und simulieren das Verhalten von Neuronen und Synapsen.
...
==== Hailo TPU
Die Hailo Beschleuniger Module sind moderner als die Lösungen von Google und erreichen eine höhere Leistungsfähigkeit. Die in diesem Projekt verwendeten Beschleuniger haben 13 und 26 TOPS zu 79.90 Fr.@noauthor_raspberry_nodate-1 resp. zu 122.90 Fr.@noauthor_raspberry_nodate. Die folgende @hailo_on_rpi5 zeigt ein Hailo Beschleuniger auf einem Raspberry Pi.
...
Die technische Architektur des Systems verbindet die Kamera mit einem leistungsstarken Backend. Die Kamera, bestehend aus einem Raspberry Pi und einer angeschlossenen Kameraeinheit, erfasst die Bilder und überträgt diese über einen Access Point an das Backend. Auf dem Backend Server laufen die rechenintensiven Analysen, welche die empfangenen Bildern nach Bestäuber untersucht. 
...
=== Bestäuber Analyse <referenz_use_case>

Die Bestäuber Detektion erfolgt in den folgenden Schritten. Ein Foto von mehreren Blüten, zum Beispiel von einem Blumentopf, durchläuft in einem ersten Schritt eine Machine Learning Analyse zur Detektion von Blüten. Die auf dem Bild detektierten Blüten auf @mw_pipeline in rot gekennzeichnet, werden anschliessen ausgeschnitten und somit zu einzelnen kleinen Bilder. Jedes dieser Bilder wird anschliessend mit einer weiteren Analyse und einem anderen Machine Learning Modell auf Bestäuber untersucht. Eine grosse Herausforderung in diesem Setup ist, dass die Anzahl Bestäuber-Analyse mit der Anzahl detektierten Blüten steigt. Dies kann bei grossen Blütenzahlen zu langen Inferenzzeiten eines einzigen Bildes führen.
...
Um Bestäuber auf einer Blüte zu detektierten, ist ein Intervall von 15 Sekunden zur Erfassung von Bildern einzuhalten. Dies stellt in etwa die Zeit dar, auf dem sich ein Bestäuber auf einer Blüte befindet.
