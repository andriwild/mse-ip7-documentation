...
Citizen Science Projekte sind ein wichtiger Bestandteil der Forschung. In der Schweiz findet man beispielsweise auf der Seite schweizforscht.ch @noauthor_vorhaben_nodate eine Sammlung von aktiven Projekten. Besonders in der Pflanzen- und Tierwelt sind solche Projekte verbreitet, weil die Beobachtungen in der Natur häufig aufwändiger sind als in anderen Bereichen. 
...
Eine Spezialität des ONNX-Frameworks ist die Fähigkeit, Modelle von anderen Formaten in das ONNX-Format zu konvertieren. Ebenso lassen sich ONNX-Modelle wieder in andere Formate exportieren. Dies führt dazu, dass ONNX als eine Art Drehscheibe zwischen den verschiedenen Frameworks fungiert  wie auf @onnx_model_convertion dargestellt @ultralytics_onnx_nodate.


#figure(
  image("../figures/onnx_model_convertion.png", width: 50%),
  caption: [ ONNX Framework als Drehscheibe\
...
==== Ultralytics
Ultralytics @ultralytics_home_nodate ist ein auf PyTorch basierendes Open-Source-Framework, das vor allem für die Entwicklung von Modellen zur Objekterkennung bekannt ist. Es wurde durch die Implementierung von YOLOv5 (You Only Look Once) @ultralytics_yolov5_nodate populär und bietet eine benutzerfreundliche Umgebung für das Training und die Bereitstellung von Objekterkennungsmodellen. Inzwischen sind neuere Implementierungen der YOLO-Familie verfügbar und mit Ultralytics anwendbar.
...
#figure(
  ...
  caption: [Disziplinen des Machine Learnings\
...
Während für eine GPU grundsätzlich keine Vorbedingungen an das Modell gestellt werden, ist dies bei der TPU und NPU durchaus möglich. Dabei spielen zwei Strategien eine wesentliche Rolle: Quantisieren und Pruning @noauthor_sparsification_nodate. Beim Quantisieren werden die Fliesskommazahlen der Gewichte zu Ganzzahlen konvertiert, wodurch die Präzision abnimmt. Dies reduziert den Speicherbedarf und ermöglicht eine schnellere Ausführung, weil einfachere Rechenoperationen ausgeführt werden können. Beim Pruning werden unwesentliche Verbindungen im Modellnetz entfernt, wodurch sich wiederum die Anzahl der Rechenoperationen reduziert. Die Ansätze werden auch kombiniert angewendet.
...
#figure(
    ...
    caption: [Hailo Accelerator auf Raspberry Pi 5\
    (Quelle: https://www.pi-shop.ch/raspberry-pi-ai-hat-13t, aufgerufen am 22.01.2025)
],
)<hailo_on_rpi5>

==== AI-Kamera
Eine weitere Kategorie sind AI-Kameras, die mit integrierten ML-Beschleuniger ausgestattet sind. Diese Kameras, wie auf @ai_cam dargestellt, können nicht nur Bilder aufnehmen, sondern auch direkt auf der Kamera Inferenzresultate berechnen, beispielsweise durch die Erkennung und Klassifikation von Objekten. Raspberry Pi stellt eine solche Kamera zur Verfügung für 77,90 Fr. @noauthor_raspberry_nodate-2.
...
#figure(
  ...
  caption: [Raspberry Pi AI-Kamera Architektur\
...
Das IoT-Toolkit umfasst unter anderem eine Kamera, die in regelmässigen Abständen Fotos von Blüten aufnimmt, um darauf Bestäuber zu detektieren. Im Rahmen des Projekts wurden so insgesamt 1,5 Millionen Bilder generiert. @pollinator_cam zeigt eine im Feld aufgestellte IoT-Kamera.
