...
Das Unterkapitel Evaluation stellt anschliessend verschiedene Resultate zu Untersuchungen von ML Frameworks vor. Zum Abschluss von diesem Kapitel werden verschiedene Möglichkeiten von Pipeline Setups vorgestellt.
...
Ein weiterer wichtiger Punkt ist die Kühlung des Systems, ansonsten wird die CPU Leistungs des Rechners gedrosselt. In der folgenden @comp_active_cooled_rpi5 ist ersichtlich, dass die Inferenzzeiten mit Kühlung rund 20% Prozent schneller gegenüber der Ausführung ohne sind.
...
Es ist sofort ersichtlich, dass die Inferenz auf der Edge TPU (4TOPS) um ein vielfaches schneller ist als auf der CPU mit NCNN des Raspberry Pi 5. Allerdings soll hier erwähnt sein, dass die EdgeTPU nur quantisierte TensorFlow lite Modelle ausführen kann und somit besteht auch ein Verlust der Präzision gegenüber des NCNN Modells. Den genauen Verlust wurde an dieser Stelle nicht eruiert, da Anwendungen auf dieser Hardware-Komponente im Anschluss nicht weiter verfolgt wurden.
...
Für die Messungen der Inferenzzeit ist das von Hailo zur Vergügung gesetllte ML-Framework verwendet worden. Zudem müssen die Modelle im Hailo eigenen Format .hef konvertiert sein, um sie auf den Beschleuniger auszuführen. Hailo stellt ein Model Zoo @tapuhi_hailo_2021 zur Verfügung, welcher die Modelle YOLOv5s und YOLOv5m beinhaltet.
Die folgende @comp_hailo_edge_tpu zeigt den Vergleich mit der Coral Edge TPU (4TOPS).
